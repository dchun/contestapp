...
run command
```
git checkout -b ch04_01_account_rework

rails g migration AccountRework
```

add to db/migrate/[timestamp]_account_rework.rb
```ruby
...
add to config/environments/productions.rb
```
...
heroku config:set SHOPIFY_SHARED_SECRET=your_shopify_shared_secret

heroku config:set DOMAIN=your_heroku_domain
```

modify app/services/shopify_integration.rb
```ruby
